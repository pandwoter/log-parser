...
RSpec.describe LogsParser::Adapters::FileAdapter do
  ...
  let(:line_entity)       { double(LogsParser::Entries::Line) }
  let(:path_creation_lib) { double(Pathname) }

  let(:line_entity_instance)       { instance_double(LogsParser::Entries::Line) }
  let(:statistic_entity_instance)  { instance_double(LogsParser::Entries::Statistic) }
